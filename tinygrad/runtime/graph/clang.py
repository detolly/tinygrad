    ...
    for ji in jit_cache:
      ...
      for buf in ji.bufs:
        ...
        if buf in input_rawbuffers:
          args.append((render_dtype(buf.dtype), f"arg{input_rawbuffers.index(buf)}", False))
        else:
          args.append((render_dtype(buf.dtype), f"({render_dtype(buf.dtype)}*)0x{ctypes.addressof(buf._buf):X}", True))
      variables = [x for x in cast(CompiledRunner, ji.prg).p.vars]
      args = args + [(render_dtype(v.dtype), v.expr, False) for v in variables]
      code.append(f"  typedef void kernel{i}_t({",".join([f"{arg[0]}{"*" if arg[2] else ""}" for arg in args])});\n" + \
        f"  ((kernel{i}_t*)0x{cast(CompiledRunner, ji.prg).clprg.buf:X})({','.join([arg[1] for arg in args])});")
      i+=1
    code.append("}")
    if DEBUG >= 4: print("\n".join(code))
